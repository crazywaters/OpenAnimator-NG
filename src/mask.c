...
static Errcode paste1_mask(void *data, int ix, int intween, int scale, Autoarg *aa)
{
	...
	if ((err = make_render_cashes()) >= 0) {
		render_mask_blit(mask_rast->bm.bp[0], mask_rast->bm.bpr, 0, 0, vb.pencel, 0, 0,
...
{
	pj_mask1blit(mask_rast->bm.bp[0], mask_rast->bm.bpr, 0, 0, vb.pencel, 0, 0, mask_rast->width,
				 mask_rast->height, color);
	zoom_it();
}
