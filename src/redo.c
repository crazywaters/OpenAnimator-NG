...
static Errcode redo_sep(Redo_rec *r)
{
	...
	sep = &r->p.sep_p;
	sep->ctable = pj_malloc(sep->ccount);
	if (sep->ctable == NULL) {
		return Err_no_memory;
	}

	err = read_gulp(rbf_name, sep->ctable, (long)sep->ccount);
	if (err < Success) {
		return err;
	}
...
#define m (&r->p.move_p)
	...
	err = clip_celrect(vb.pencel, &m->orig, &clipcel);
	if (err < 0) {
		return err;
	}
	if (m->clear_move_out) {
		pj_set_rect(vb.pencel, vs.inks[0], m->orig.x, m->orig.y, m->orig.width, m->orig.height);
	}
	clipcel->x = m->new.x;
	clipcel->y = m->new.y;
	...
	(*get_celblit(vs.render_one_color))(clipcel, 0, 0, vb.pencel, clipcel->x, clipcel->y,
										clipcel->width, clipcel->height, &xld);
	zoom_cel(clipcel);
	if (m->clear_move_out) {
		do_leftbehind(m->orig.x, m->orig.y, clipcel->x, clipcel->y, m->orig.width, m->orig.height,
