...
#define y1 ends[1].y
	...
	if (rb->type == LINE_BRUSH) {
		...
		err = (filled_polygon(&bl_poly, render_hline, vb.pencel, poly_cline_with_render_dot, NULL));
		goto done;
	}
...
#define bmap ((Bytemap *)rb->rast)
		...
		for (by = 0; by < rb->height; ++by) {
			...
			for (;;) {
				...
				dcur = FSUB(fpy, FMUL(fpx, m_inv));
				if (dcur < 0) {
					dcur = -dcur;
				}

				/* get absolute value of dy from line in fpx as temp */
				fpx = FSUB(FMUL(fpy, m), fpx);
				if (fpx < 0) {
					fpx = -fpx;
				}
				...
				numsame = pj_bsame(line + bx, rb->width - bx) - 1;
				if (0 == numsame) {
					break;
				}
				bx += numsame;
...
/*
 * Renders a line using the current brush
 * Seems to be the main entry point for line draws.
 */
