...
bool make_simple_cfit(Cmap *scmap, Cmap *dcmap, Celcfit *cfit, int clearc)
{
	...
	if ((cmap_cksum != cfit->src_cksum) || (dcmap_cksum != cfit->dst_cksum) ||
		(cfit->ink0 != clearc)) {
...
bool make_render_cfit(Cmap *scmap, Celcfit *cfit, SHORT tcolor)
{
	...
	if (vs.render_one_color) {
		if ((vs.ccolor != cfit->ccolor) || (tcolor != cfit->tcolor) || (cfit->ink0 != vs.inks[0])) {
		...
		cfit->ccolor = -1;
	} else if (vs.render_under && vs.zero_clear && vs.inks[0] != tcolor) {
		int i;
