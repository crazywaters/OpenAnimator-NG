...
{
	Errcode err = check_poly_file(source);
	if (err < Success) {
		return err;
	}
...
static void qload_path(void)
{
	...
	char *path = vset_get_filename(stack_string("load_path", buf),
							".PLY",load_str,OPTPATH_PATH,NULL,0);
...
{
	char buf[PJ_PATH_MAX];
	char *path = vset_get_filename(stack_string("save_path", buf),
							   ".PLY",save_str,OPTPATH_PATH,NULL, true);
...
{
	char buf[PJ_PATH_MAX];
	char *path = vset_get_filename(stack_string("load_poly",buf),
								".PLY",load_str,POLY_PATH,NULL,0);
...
{
	Errcode err = test_load_tween(name);
...
static void fml_kill(void)
{
	...
	char *ends;
	char buf1[PJ_PATH_MAX], buf2[16];

	hide_mp();

	ends = file_ends[vs.file_type];
	parse_to_semi(&ends, suffix, sizeof(suffix));
	path = vset_get_filename(stack_string("del_file", buf1),
							suffix,
							stack_string("del_str", buf2),
							file_paths[vs.file_type],NULL,0);
	if (path != NULL) {
		if(!pj_exists(path)) {
			cant_find(path);
		}
...
void go_files(int file_type)
{
	if(file_type >= 0) {
		vs.file_type = file_type;
	}
	same_old_files();
}
