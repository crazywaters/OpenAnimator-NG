...
static void draw_stringq(Sqwork *qw)
{
	...
	gftext(&qw->cb, qw->f, string, qw->textx, qw->texty, qw->sdetail, TM_MASK1, qw->sblock);
	qw->curson = 0;
}
...
static int stringq_xor_cursor(Sqwork *qw)
{
	...
	pj_xor_rect(
		m->root, qw->sblock ^ qw->sdetail,
		qw->cb.x + qw->textx + fnstring_width(qw->f, sq->string + sq->dpos, sq->cpos - sq->dpos),
...
static bool right_scroll(Sqwork *qw)
{
	...
	while (fnstring_width(qw->f, sq->string + sq->dpos, sq->cpos - sq->dpos + 1) >=
		   b->width - 2 * MB_IBORDER - qw->textx) {
		sq->dpos += 1;
		ret = true;
	}
	return (ret);
