...
static void twe_see_both_ends(void)
{
	...
	switch (vs.tween_end) {
		case TWEEN_BOTH:
		case TWEEN_START:
			rub_poly_points(p0, vs.closed_curve, sc, sc, PTCOL, PT1COL);
			break;
		case TWEEN_END:
	...
	switch (vs.tween_end) {
		case TWEEN_BOTH:
		case TWEEN_END:
			rub_poly_points(p1, vs.closed_curve, ec, ec, PTCOL, PT1COL);
			break;
		case TWEEN_START:
...
static Errcode tween_sv_undo(void)
{
	...
	if ((err = clone_dl_list(&twcb->cur.links, &twcb->old.links, sizeof(Tween_link))) < Success) {
		return (err);
	}
...
static void twtw_wire(int mode)
{
	find_range();
	zoom_unundo();
	a_wireframe_tween(&twcb->cur, tr_frames, flix.hdr.speed, vs.ccolor, vs.ccolor, vs.closed_curve,
					  mode);
	twe_see_both_ends();
}
...
static void tween_do_star(Poly *p, int startype)
{
	...
	if (p != NULL) {
		clear_struct(&rp);
		if ((err = rub_keep_star_type(&rp, startype, get_pcolor(p), &theta, &rad, &rad2)) >=
			Success) {
			free_polypoints(p);
			*p = rp;
...
static void tti_polygon(void)
{
	...
	if ((this = new_poly_point(p)) != NULL) {
		make_poly_loop(p, vs.tween_spline, vs.closed_curve, this, get_pcolor(p));
		p->polymagic = POLYMAGIC;
		softerr(force_other(p), NULL);
		redraw_both_ends();
	}
...
static tti_func tti_vectors[] =
	...
	{
		tti_polygon, tti_shape,  tti_star,   tti_petal,
		tti_rpoly,   tti_oval,   tti_mpoint, /* move point */
		tti_magnet,                          /* magnet */
		tti_blow,                            /* blow */
		tti_mshape,  tti_mtween,             /* mtween */
		tti_sshape,                          /* sshape */
		tti_stween,                          /* stween */
		tti_link,                            /* link */
};
...
/* returns whether you should grey out 'render' option */
...
static Errcode save_tween_state(void)
{
	...
	return softerr(save_tween(tween_name, &twcb->cur), "!%s", "tween_sstate", tween_name);
}
...
Pentool tween_pen_tool = PTOOLINIT1(
	NONEXT, NOTEXT,             /* "Tween", loaded by load_tween_panel_strings() */
	PTOOL_OPT, TWEEN_PTOOL, "", /* no help text used!!!! "Transform one shape into another.", */
	NO_SUBOPTS, NULL, twe_tool_func, &plain_ptool_cursor.hdr, NULL, /* on install */
	NULL                                                            /* on remove */
);
...
static void tween_pull_disables(Menuhdr *mh)
{
	static SHORT notween_pulltab[] = {
		...
		TWE_CLE_PUL, TWE_END_PUL, SHA_SAV_PUL, SHA_REV_PUL,
	};
...
static void tween_pull_asterisks(Menuhdr *mh)
{
	static SHORT twtool_pulltab[] = {
		...
		MOV_MAG_PUL, MOV_BLO_PUL, MOV_MVS_PUL, MOV_MVT_PUL, MOV_SZS_PUL, MOV_SZT_PUL, MOV_LIN_PUL,
	};
...
static void twe_color_refresh(void *dat, USHORT why)
{
	...
	if (flx_olays_hidden() || flxtime_data.draw_overlays != tween_redraw_with_data) {
		return;
	}

	if (os != twcb->s_color || oe != twcb->e_color) {
		twe_see_both_ends();
	}
}
...
void tween_menu(bool renderable /* grey out render button? */)
{
	...
	fliborder_on();
	if (load_soft_pull(&tpull, 7, "tween", TWEENP_MUID, tween_selit, tween_dopull) < Success) {
		goto OUT;
	}
