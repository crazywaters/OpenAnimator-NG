...
Errcode transpblit(Rcel *tcel, int clearcolor, int clear, int tinting)
{
	...
	err = tblit(tcel, 0, 0, vb.pencel, tcel->x, tcel->y, tcel->width, tcel->height, clearcolor,
				clear, tinting, vl.ink->dither, tcel->cmap, NULL);
	free_bhash();
	return err;
}
						   ...
						   SHORT h, Tcolxldat *txd, Cmap *scmap)
{
	...
	switch (vl.ink->ot.id) {
		...
		case tsp_INKID: /* transparent */
			...
			err = tblit(src, sx, sy, dest, dx, dy, w, h, tcolor, vs.zero_clear, vl.ink->strength,
						vl.ink->dither, scmap, xlat);
			rect_zoom_it(dx, dy, w, h);
			break;
		default:
		...
		for (curx = x; curx < endx; ++curx) {
			...
			if (curalpha) {
				if (curalpha == 255) {
					...
					pj_put_dot(r, closestc(&dest, ctab, cmap->num_colors), curx, y);
				}
			}
		}
		alpha += abpr;
...
Errcode render_disk(Raster *r, SHORT cenx, SHORT ceny, SHORT diam)
{
	...
	start_abort_atom();
	err =
		doval(cenx, ceny, diam, r->aspect_dx, r->aspect_dy, NULL, NULL, poll_render_hline, r, true);
	free_render_cashes();
	return (errend_abort_atom(err));
}
...
void render_brush(SHORT x, SHORT y)
{
	...
	if (vl.ink->ot.id == opq_INKID && !(vs.use_mask || vs.make_mask)) {
		...
		rect_zoom_it(x - rb->cent.x, y - rb->cent.y, rb->width, rb->height);
		return;
	}
...
Errcode render_circle(Raster *r, SHORT cenx, SHORT ceny, SHORT diam)
{
	...
	doval(cenx, ceny, diam, r->aspect_dx, r->aspect_dy,
		  vs.use_brush ? render_brush_with_data : render_dot, vb.pencel, NULL, NULL, false);
	free_render_cashes();
	return Success;
}
