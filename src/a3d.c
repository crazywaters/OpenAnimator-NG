...
static Errcode poly3_init(Poly3 *p, int count, Short_xyz *points)
{
	clear_struct(p);
	if (points == NULL) {
		p->p3alloc = p->p3 = pj_malloc(sizeof(*points) * count);
		if (p->p3alloc == NULL) {
			poly3_cleanup(p);
			return Err_no_memory;
		}
...
static Errcode ado_load_element(void)
{
	...
	if (got_path) {
		err = load_a_poly(ppoly_name, &ado_path_poly);
		if (err < Success) {
			goto ERR;
		}
	...
	switch (vs.ado_source) {
		case OPS_POLY:
		case OPS_SPLINE:
			err = load_a_poly(poly_name, &ado_poly_el);
			if (err < Success) {
				goto ERR;
			}
			break;
		case OPS_TWEEN:
			...
			err = load_tween(tween_name, &ado_tween_el);
			if (err < Success) {
				goto ERR;
			}
			err = ts_to_tw_list(&ado_tween_el, (vs.closed_curve || vs.fillp), &ado_tlist);
			if (err < Success) {
				goto ERR;
			}
			break;
		   ...
		   Poly *path_poly, int path_type, bool path_closed) /* We got a path to cope with too? */
{
	...
	if (path_poly != NULL) {
		if (path_type == 0) /* splining... */
		{
			...
			err = calc_path_pos(path_poly, &path_point, scale, path_closed);
			if (err < 0) {
				return err;
			}
		}
...
static Errcode ado_transform(Short_xyz *points, int count, int scale, Short_xy *dest)
{
	...
	while (as != NULL) {
		err = move_vpoly(points, points, count, as, scale, path, vs.ado_path, vs.pa_closed);
		if (err < Success) {
			return err;
		}
	...
	err = calc_zpoly(points, dest, count, vb.pencel->width >> 1, vb.pencel->height >> 1,
...
static Errcode ado_calc_poly(Poly2 *dpoly, Rcel *form, SHORT scale)
{
	...
	switch (vs.ado_source) {
		case OPS_SPLINE:
		case OPS_POLY:
			err = poly3_from_poly(&vpoly, &ado_poly_el);
			if (err < Success) {
				goto OUT;
			}
			break;
	...
	err = poly2_init(dpoly, vpoly.count);
	if (err < Success) {
		goto OUT;
	}
...
static Errcode twirl1(void *celcfit, int ix, int frames, int scale, Autoarg *aa)
{
	...
	switch (vs.ado_source) {
		case OPS_SCREEN: /* screen */
			if (vs.ink_id != opq_INKID) {
				err = report_temp_save_rcel(&oundo, undof);
				if (err < Success) {
					goto error;
				}
				pj_set_rast(undof, vs.inks[0]);
			}
			tf = clone_pencel(vb.pencel);
			if (tf == NULL) {
				goto nomem_error;
			}
	...
	err = ado_load_element();
	if (err < 0) {
		goto error;
	}

	err = ado_calc_poly(&dpoly, tf, scale);
	if (err < 0) {
		goto error;
	}
...
render_rast:
			...
			load_poly_minmax(&xf);
			err = render_transform(tf, &xf, &tcxl);
			if (err < 0) {
				goto error;
			}
		...
		case OPS_POLY:
		case OPS_SPLINE:
			err = rado_poly(dpoly.p2, dpoly.count, vs.ado_source == OPS_SPLINE);
			if (err < Success) {
				goto error;
			}
			break;
		case OPS_TWEEN:
			err = rado_poly(dpoly.p2, dpoly.count, vs.tween_spline);
			if (err < Success) {
				goto error;
			}
			break;
...
static Errcode dcenter(dotout_func dotout, void *dotdat, int scale)
{
	...
	err = ado_transform(cdvecs, 4, scale, cdpts);
	if (err >= 0) {
		for (i = 1; i < 4; i++) {
			pj_cline(cdpts[0].x, cdpts[0].y, cdpts[i].x, cdpts[i].y, dotout, dotdat);
		}
	}
	return err;
}
...
static void marqi_ado_poly(Marqihdr *mh, Poly2 *dpoly, int scale)
{
	...
	msome_vector(dpoly->p2, dpoly->count, mh->pdot, mh, is_vector() && !vs.closed_curve,
				 sizeof(dpoly->p2[0]));
}
...
static Errcode ado_preview(bool doloop)
{
	...
	do {
		for (i = 0; i < tr_frames; i++) {
			...
			/* calc new 2D poly */
			err = ado_calc_poly(&dpoly, ado_cel, scale);
			if (err < 0) {
				goto OUT;
			}
			if (!first) {
				/* erase old wireframe image */
			...
			while (clock > pj_clock_1000()) {
				/* Wait until it's time to advance. */
				wait_sync();
			}
			if (clock < pj_clock_1000()) {
				/* Let clock slip if we're slow. */
				clock = pj_clock_1000();
			}
			if (err < Success) {
				/* Get out of here on error or abort. */
				goto OUT;
			}
...
static int calc_see_ado_poly(Rcel *cel, SHORT scale)
{
	...
	err = ado_calc_poly(&dpoly, cel, scale);
	if (err >= 0) {
		vinit_marqihdr(&mh, 1, 1);
		marqi_ado_poly(&mh, &dpoly, scale);
	}
...
static Errcode make_path(void)
{
	...
	clear_struct(&wpoly);
	err = report_temp_save_rcel(&opic, vb.pencel);
	if (err < Success) {
		return err;
	}
	make_rot_op();
	...
	if (err >= 0) {
		...
		if (JSTHIT(MBPEN)) {
			...
			switch (vs.ado_path) {
				...
				case PATH_CLOCKED:
					find_range();
					sample_path(&wpoly, millisec_to_jiffies(flix.hdr.speed), tr_frames, 1);
					break;
			}
			curveflag = 0;
			if (wpoly.pt_count > 0) {
...
#undef ZPOS
	...
	make_rot_op();
	get_ado_cel();
	err = ado_calc_poly(&dpoly, ado_cel, SCALE_ONE);
	if (err < 0) {
		goto OUT;
	}
	vinit_marqihdr(&mh, 1, 1);
...
static Pentool ado_mouse_ptool = PTOOLINIT1(NONEXT, empty_str, /* real name filled in later */
											PTOOL_OPT, 0, NULL, NO_SUBOPTS, NULL, ado_mouse_ptfunc,
											&plain_ptool_cursor.hdr, NULL, /* on install */
											NULL                           /* on remove */
);
...
/* Let's go to the optics editor folks! */
void go_ado(void)
{
	Menuhdr tpull;
	...
	if (load_soft_pull(&tpull, 10, "optics", A3DP_MUID, ado_selit, do_a3dpull) >= Success) {
