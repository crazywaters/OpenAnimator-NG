...
{
	if ((JSTHIT(KEYHIT) && (UBYTE)icb.inkey == ' ') || (rclick_on_screen())) {
		toggle_menu();
		return true;
	} else {
		return false;
...
Vscoor scale_vscoor(SHORT coor, SHORT relto)
{
	...
	tcoor *= ((double)coor);
	tcoor = ceil(tcoor);
	if (tcoor > VS_MAXCOOR * 2) {
		/* overflow */
		return VS_MAXCOOR * 2;
	}
	if (tcoor < -(VS_MAXCOOR * 2)) { /* underflow */
		return -VS_MAXCOOR * 2;
	}
	return (SHORT)tcoor;
}
...
static SHORT fliwin_width_rel[] = {INITOSENT(flicentx, flicent.x),    INITOSENT(gridx, grid.x),
								   INITOSENT(gridw, grid.width),      INITOSENT(rgcx, rgc.x),
								   INITOSENT(expand_x, expand_pos.x), -1};

static SHORT fliwin_height_rel[] = {INITOSENT(flicenty, flicent.y),    INITOSENT(gridy, grid.y),
									INITOSENT(gridh, grid.height),     INITOSENT(rgcy, rgc.y),
									INITOSENT(expand_y, expand_pos.y), -1};

static SHORT screen_width_rel[] = {INITOSENT(quickcentx, quickcent.x),
								   INITOSENT(zwincentx, zwincent.x), INITOSENT(zwinw, zwinw), -1};

static SHORT screen_height_rel[] = {INITOSENT(quickcenty, quickcent.y),
									INITOSENT(zwincenty, zwincent.y), INITOSENT(zwinh, zwinh), -1};
static SHORT flidiag_rel[] = {INITOSENT(rgr, rgr), -1};
...
void save_undo(void)
{
	pj_blitrect(vb.pencel, 0, 0, undof, 0, 0, undof->width, undof->height);
	pj_cmap_copy(vb.pencel->cmap, undof->cmap);
}

void zoom_unundo(void)
{
	pj_blitrect(undof, 0, 0, vb.pencel, 0, 0, vb.pencel->width, vb.pencel->height);
	zoom_it();
