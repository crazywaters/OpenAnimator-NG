...
static double deltaE2000(LabColor lab1, LabColor lab2) {
	...
	// T factor
	double T = 1.0 - 0.17 * cos(deg2rad(avgHPrime - 30.0))
				   + 0.24 * cos(deg2rad(2.0 * avgHPrime))
	...
	double deltaE = sqrt(pow(deltaLPrime / S_L, 2) +
						 pow(deltaCPrime / S_C, 2) +
						 pow(deltaHPrime / S_H, 2) +
...
SDL_Surface* sdlpdr_convert_surface_to_palette(SDL_Surface* surface, SDL_Palette* palette)
{
	...
	SDL_SetSurfacePalette(indexedSurface, palette);
	for (int y = 0; y < surface->h; y += 1) {
		for (int x = 0; x < surface->w; x += 1) {
			Uint8 r, g, b;
			SDL_ReadSurfacePixel(surface, x, y, &r, &g, &b, NULL);
			uint64_t min_dist = INT64_MAX;
			int best_index = -1;
			for (int index = 0; index < palette->ncolors; index += 1) {
				const Uint8 delta_r = r - palette->colors[index].r;
				const Uint8 delta_g = g - palette->colors[index].g;
				const Uint8 delta_b = b - palette->colors[index].b;

				uint64_t dist = delta_r*delta_r + delta_g*delta_g + delta_b*delta_b;
				if (dist < min_dist) {
					min_dist = dist;
					best_index = index;
				}
			...
			SDL_WriteSurfacePixel(
				indexedSurface, x, y,
				palette->colors[best_index].r,
				palette->colors[best_index].g,
				palette->colors[best_index].b,
				0xFF);
		}
	}
