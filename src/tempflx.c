...
static Errcode open_flx(char* path, Flxfile* flx, enum XReadWriteMode mode)
{
	LONG err;
	long acc;
		...
		switch (pd.type) {
			...
			case FP_CELDATA:
				if (pd.fchunk.size != sizeof(Celdata)) {
					softerr(Err_unimpl, "!%d%d", "tflx_celdata", sizeof(Celdata), pd.fchunk.size);
					break;
				}
		}
	}
...
{
	if (soft_yes_no_box("!%s%d%d", "load_abort", name, flix.hdr.frame_count, frame_count)) {
		return (Err_abort);
	}
	return (Success);
}
 ...
 * return error. */
{
	...
	while (flix.hdr.frame_count > 0) {
		...
		if ((err = ring_tflx(frame)) >= Success) {
			...
			cleanup_wait_box();
			softerr(reason, "!%s%d%d", "tflx_ring", name, flix.hdr.frame_count, frame_count);
			break;
		}
