	...
	if (td->tcount) {
		...
		switch (td->ttype) {
			...
			case 2:
				...
				pj_tblitrect(vb.pencel, 0, 0, td->tscreen, 0, 0, vb.pencel->width,
							 vb.pencel->height, ink);
...
static Errcode auto_shrink1(void *data, int ix, int intween, int scale, Autoarg *aa)
{
	...
	for (y = 0; y < vb.pencel->height; y += incy) {
		...
		for (x = 0; x < vb.pencel->width; x += incx) {
			...
			rgb.g = (rgbacc[1] >>= 2);
			rgb.b = (rgbacc[2] >>= 2);
			pj_put_dot(vb.pencel, bclosest_col(&rgb, COLORS, dither), x >> 1, y >> 1);
		}
	}

	free_bhash();

	pj_set_rect(vb.pencel, vs.inks[0], 0, vb.pencel->height >> 1, vb.pencel->width,
				vb.pencel->height >> 1);

	pj_set_rect(vb.pencel, vs.inks[0], vb.pencel->width >> 1, 0, vb.pencel->width >> 1,
				vb.pencel->height >> 1);
...
static Errcode dall(Autoarg *aa)
{
	...
	if (aa->doing_overlays) {
		/* just building new file from overlays */
		xf = undof;
	} else {
		err = alloc_pencel(&xf);
		if (err < 0) {
	...
	for (i = 0; i < flix.hdr.frame_count; i++) {
		...
		if (aa->doing_overlays) {
			...
			if (i == 0) {
				save_undo();
				err = flx_ringseek(undof, vs.frame_ix, 0);
				if (err >= Success) {
					zoom_unundo();
		...
		} else /* doing an avec function */
		{
			...
			{
				vs.ccolor = closestc(&aa->auto_rgb, vb.pencel->cmap->ctab, COLORS);
				vs.inks[7] = closestc(&aa->auto_8rgb, vb.pencel->cmap->ctab, COLORS);
			}

			if (aa->flags & AUTO_USES_UNDO) {
				cbuf = pj_malloc(cbufsz);
				if (cbuf == NULL) {
					goto nomem_error;
				}
			...
			err = auto_apply(aa, (i == flix.hdr.frame_count ? 0 : i), flix.hdr.frame_count);
			if (err < 0) {
				goto error;
			}
		...
		if (!do_compress) {
			err = read_flx_frame(&flix, cbuf, i);
			if (err < 0) {
				goto error;
			}
			...
			ssize = pj_fli_comp_cel(cbuf, xf, vb.pencel, COMP_DELTA_FRAME, flix.comp_type);
		}
...
static int calc_ease_in(int time_ix, int time_frames)
{
	...
	if (time_ix < time_frames / 2) {
		if (time_ix == 0) {
			...
			time_scale = sine_scale(time_ix, time_frames), time_scale = rscale_by(time_scale, 7, 9);
		}
	} else {
...
int calc_time_scale(int ix, int intween)
{
	...
	if (vs.ado_tween) {
		if (vs.ado_ease && vs.ado_ease_out) {
			...
			time_scale = SCALE_ONE - calc_ease_in(time_frames - time_ix, time_frames);
		} else {
...
Errcode auto_apply(Autoarg *aa, int ix, int intween)
{
	...
	err = (*aa->avec)(aa->avecdat, ix, intween, calc_time_scale(ix, intween), aa);
	if (err < 0) {
		return err;
	}
	zoom_it();
