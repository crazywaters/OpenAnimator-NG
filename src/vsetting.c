...
Errcode reload_tsettings(Vsettings *pvs, Vset_flidef *fdef)
{
	...
	err = read_gulp(tsettings_name, buf, (long)sizeof(*buf));
	if (err < Success) {
		goto error;
	}
...
Errcode vset_get_pathinfo(int ptype, Vset_path *pathinfo)
{
	...
	if (((unsigned int)ptype) < VSET_NUM_PATHS && (err = reopen_tsettings(&vsf)) >= Success) {
		err = xffreadoset(vsf.xf, pathinfo, path_type_offset(&vsf, ptype), sizeof(Vset_path));
		close_vsetfile(&vsf);
	} else {
	...
	if (pathinfo->scroller_top == 0 && pathinfo->path[0] == 0 && pathinfo->wildcard[0] == 0) {
		switch (ptype) {
			case JOIN_PATH:
			case JOIN_MASK_PATH:
				ptype = FLI_PATH;
...
Errcode vset_set_pathinfo(int ptype, Vset_path *pathinfo)
{
	...
	*pj_get_path_suffix(name) = 0;
	...
	err = reopen_tsettings(&vsf);
	if (err >= Success) {
		err = xffwriteoset(vsf.xf, pathinfo, path_type_offset(&vsf, ptype), sizeof(Vset_path));
		close_vsetfile(&vsf);
	}
	strcpy(name, save_name);
	return err;
...
static Errcode save_settings_file(char *path, bool full_defaults)
{
	...
	err = write_settings_chunk(newxf, VSETFILE_MAGIC, 0, full_defaults ? vb.pencel->cmap : NULL,
							   false);
	xffclose(&newxf);
	if (err < Success) {
		pj_delete(path);
								   ...
								   Cmap *cmap, bool load_mucolors, bool as_defaults)
{
	...
	init_chunkparse(&pd, xf, DONT_READ_ROOT, offset, sizeof(Fat_chunk), id->size);
...
read_chunk:
				...
				recsize = buf->size;
				err = read_parsed_chunk(&pd, buf, recsize);
				if (err < Success) {
					goto error;
				}
	...
	err = write_gulp(tsettings_name, tset, sizeof(*tset));
	if (err < Success) {
		goto error;
	}

	if (load_mucolors) {
		memcpy(vconfg.mc_ideals, tset->vslow.mc_ideals, sizeof(vconfg.mc_ideals));
	}
...
static Errcode load_fli_settings(char *path, Cmap *cmap)
{
	...
	for (;;) {
		if (get_next_chunk(&pd)) {
			...
			err =
				load_settings_chunk(flif.xf, &pd.fchunk, pd.chunk_offset, NULL, NULL, true, false);
			break;
		}
...
static Errcode load_file_settings(char *path, Vset_flidef *fdef, bool default_reset)
{
	...
	if (id.type == VSETFILE_MAGIC) {
		err = load_settings_chunk(xf, &id, 0, fdef, default_reset ? vb.pencel->cmap : NULL, true,
								  default_reset);
...
static Errcode load_vsettings(char *path)
{
	...
	err = load_file_settings(path, NULL, false);
	if (err < Success) {
		goto error;
	}
